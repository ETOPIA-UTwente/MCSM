...
class Admin(object):
  ...
  def fireTrigger(self, output_pin, delay, pulses, pulse_width_, pulse_width_decay, start_time):
    ...
    """ counter_pulse = pulses
    last_pulse = 0
    pulse_width = float(pulse_width_)
    while time.time() < (start_time + delay):
      pass
    while counter_pulse >= 0:
      now = time.time()
      if now > (last_pulse + pulse_width):
        last_pulse = now
        if pulse_width >= pulse_width_decay: 
          pulse_width -= pulse_width_decay
        counter_pulse -=1
        if GPIO.input(output_pin) == 1:    
          while(GPIO.input(output_pin) == 1):
            GPIO.output(output_pin,GPIO.LOW)
            time.sleep(0.00001)
        else:
          GPIO.output(output_pin,GPIO.HIGH)
          while(GPIO.input(output_pin) == 0):
            GPIO.output(output_pin,GPIO.HIGH)
            time.sleep(0.00001) """
  ...
  """ def picoSetupChannelBuffer(self, channel, buffer):
    ...
    self.status[f"setDataBuffers{channel}"] = self.ps.ps5000aSetDataBuffers(self.chandle,
                                                        self.ps.PS5000A_CHANNEL[f'PS5000A_CHANNEL_{channel}'],
