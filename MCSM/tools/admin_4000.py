...
class Admin(object):
  class PS4000A_TRIGGER_INFO(Structure):
    _pack_ = 1
  ...
  def __init__(self, num_channels, sample_period, raspi=False):
    ...
    self.channel_range = 8  #Enum 0 - 13 on Page 81 of programmers guide series 4000 -> 8 = 5V range
    self.status = {}
    ...
    #self.triggerInfo = self.PS4000a_TRIGGER_INFO()
    ...
    self.raspi = False #PIGPIO used for triggering - remotely over network
    if raspi:
      self.init_raspi() #PIGPIO used for triggering - remotely over network
  ...
  def calculate_time_base(self):
    self.timebase = ctypes.c_uint32(int((( self.sample_period ) / 12.5e-9) - 1 )) # from programmers guide formula 4824a series pg 23 prog guide - 12 bit mode
  ...
  def time_units(self,sampleTimeInNS):
    ...
    self.sampleUnits = ps.PS4000A_TIME_UNITS['PS4000A_NS']
  ...
  def run_streaming(self):
    ...
    status = {}
    downsampleRatio = 1
    status["runStreaming"] = self.ps.ps4000aRunStreaming(self.chandle,
                                                    ctypes.byref(self.sampleInterval),
                                                    self.sampleUnits,
                                                    maxPreTriggerSamples,
                                                    0,#totalSamples, Not needed in continous streaming mode - used for autoStop
                                                    autoStopOn,
                                                    downsampleRatio,
                                                    self.ps.PS4000A_RATIO_MODE['PS4000A_RATIO_MODE_NONE'],
                                                    self.sizeOfOneBuffer)
    assert_pico_ok(status["runStreaming"])

  def get_latests_stream_values(self, cFuncPtr):
    status = {}
    status["getStreamingLastestValues"] = ps.ps4000aGetStreamingLatestValues(self.chandle, cFuncPtr, None)
  ...
  def fireTrigger(self, output_pin, delay, pulses, pulse_width_, pulse_width_decay, start_time):
    ...
    while True:
      ...
      if time.time() > (start_time + delay):
        ...
        break 
    """
    # PIGPIO now being used for trigger - Remotely over network
    print(f"Trigger time pulse @@@ ::: {time.time()}")
  ...
  def picoSetupHandle(self):
    ...
    #resolution = self.ps.PS4000a_DEVICE_RESOLUTION["PS4000a_DR_12BIT"]
    # Returns handle to chandle for use in future API functions
    self.status["openunit"] = self.ps.ps4000aOpenUnit(ctypes.byref(self.chandle), None)#, resolution)
                                  #ps.ps4000aOpenUnit(ctypes.byref(chandle), None)
    ...
    try:
        ...
        if powerStatus == 286:
            self.status["changePowerSource"] = self.ps.ps4000aChangePowerSource(self.chandle, powerStatus)
  ...
  def stop_and_close(self):
    status = {}
    status["stop"] = self.ps.ps4000aStop(self.chandle)
    assert_pico_ok(status["stop"])
    print("Successfully executed")
    # Disconnect the scope
    # handle = chandle
    status["close"] = self.ps.ps4000aCloseUnit(self.chandle)
  ...
  def picoSetupChannel(self, channel):
    ...
    #channel_range = self.ps.PS4000_RANGE['PS4000_5V']
    status["setCh"] = self.ps.ps4000aSetChannel(self.chandle,
                                            channel,
                                            1,
                                            2,
                                            self.channel_range,
  ...
  def getMaxSegments(self):
    ...
    status["GetMaxSegments"] = self.ps.ps4000aGetMaxSegments(self.chandle, ctypes.byref(self.MaxSegments))
  ...
  def setTimeBase(self, buffer_lenght):
    ...
    status["GetTimebase2"] = self.ps.ps4000aGetTimebase2(  self.chandle, 
                                                          self.timebase, 
                                                          buffer_lenght, 
  ...
  def getTriggerInfoBulk(self):
    ...
    # ctypes.byref(self.MaxSegments)
    status["GetTriggerInfoBulk"] = self.ps.ps4000aGetTriggerInfoBulk( self.chandle, 
  ...
  def MemorySegments(self):
    ...
    # ctypes.byref(self.MaxSegments)
    status["GetMaxSegments"] = self.ps.ps4000aMemorySegments(
                                                              self.chandle, 
                                                              self.MaxSegments, 
  ...
  def setNoCaptures(self):
    ...
    # ctypes.byref(self.MaxSegments)
    status["SetNoOfCaptures"] = self.ps.ps4000aSetNoOfCaptures(self.chandle, self.noCaptures)
  ...
  def setTrigger(self, delay):
    ...
    status["maximumValue"] = self.ps.ps4000aMaximumValue(self.chandle, ctypes.byref(self.maxADC))
    threshold = int(mV2adc(1000, self.channel_range, self.maxADC))
    print(f"Threhold : {threshold}")
    PS4000A_RISING = 2 # enum is 2 rsiing or falling
    ...
    status["SetSimpleTrigger"] = self.ps.ps4000aSetSimpleTrigger(
                                                                            self.chandle, 
                                                                            1, 
                                                                            3,#self.ps.PS4000A_CHANNEL[f'PS4000A_CHANNEL_A'], 
                                                                            threshold, 
                                                                            PS4000A_RISING, 
                                                                            delay, 
                                                                            trigger_timeout)
  ...
  def runStreaming(self, sampleInterval, sampleUnits, totalSamples, sizeOfOneBuffer):
    ...
    downsampleRatio = 1
    #status = {}
    self.status["runStreaming"] = self.ps.ps4000aRunStreaming(self.chandle,
  ...
  def picoSetupChannelBuffer(self, channel, buffer):
    ...
    self.status[f"setDataBuffers{channel}"] = self.ps.ps4000aSetDataBuffer(self.chandle,
                                                        self.ps.PS4000A_CHANNEL[f'PS4000A_CHANNEL_{channel}'],
  ...
  def getValuesTriggerTimeBulk(self):
    ...
    # segment index = 0
    # ratio mode = PS4000_RATIO_MODE_NONE = 0
    self.status[f"getValuesTriggerTimeBulk"] = self.ps.ps4000aGetValuesTriggerTimeOffsetBulk(  self.chandle,
                                                                                              ctypes.byref(self.timesUpperBits),
                                                                                              ctypes.byref(self.timesUpperBits),
                                                                                              self.ps.PS4000A_TIME_UNITS['PS4000A_NS'],
                                                                                              self.fromSegmentIndex,
                                                                                              self.toSegmentIndex)
